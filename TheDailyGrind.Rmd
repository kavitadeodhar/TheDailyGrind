...
Note: We will cover the challenges faced at the end of this markdown.

Let us start by gathering data using twitter API. 
...
##################################################################
```
We have also attempted to decode emojis used in the tweets. Considering the fact that most people may also exclusively communicate using emojis(You can now order pizza using emojis!), we decided not to drop emojis out of our analysis.Source of this data is "https://raw.githubusercontent.com/today-is-a-good-day/Emoticons/master/emDict.csv"

First we create data sets for each of the individual brands.
...
N=400  # Number tweets to request from each query
...
From the map above,we can see the distribution of tweets across the country. We see that the number of tweets for Panera look higher. The number of tweets for every brand was the same.This simply means that the latitude longitude information was available for larger number of tweets for Panera than the other brands. 
...
```
The above plot shows how the brands compare for the number of tweets per category. 
...
ACSI is the customer satisfaction indicator.
